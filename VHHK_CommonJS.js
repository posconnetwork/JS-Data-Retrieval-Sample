 ...
 */
const getDom = async (url, select = (d) => d) => new jsdom.JSDOM(select((
    // Await for a response from the URL
    await axios.default.get(url)).data)).window.document;
    ...
    switch (icao) {
        case "VHHH": {
            ...
            const [atis, metar, taf, sigmet] = [
                ...
                // METAR
                getDom(metar_url,
                    data => data.metar_decode_eng_json.content.table.content
                ).then(
                    doc => concat(doc, "table strong ~ p")
                ),

                // TAF
                getDom(taf_url,
                    data => data.taf_decode_eng_json.content1.table1.content
                ).then(
                    doc => concat(doc, "table strong ~ p")
                ),
